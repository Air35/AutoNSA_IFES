...
//        this.jTableBanco = new JTable();
    ...
    private void configureTextMet(int row){
        if(listInfos.get(row).getMetodologiaAplicada().getOpcao() == 1){
            this.jTextAreaAjuda.setText(" Metodologia escolhida: " + listInfos.get(row).getMetodologiaAplicada().getMetodologia("1")
                    + "\n Coeficiente: " + listInfos.get(row).getMetodologiaAplicada().getCoefBR());            
        }else{
            this.jTextAreaAjuda.setText(" Metodologia escolhida: " + listInfos.get(row).getMetodologiaAplicada().getMetodologia("0")
...
//        this.jLabelDataInicial.setText("Data Inicial:");
    ...
    private void configureBtns(){
     ...
     public void btAvancaractionPerformed(java.awt.event.ActionEvent evt) {
...
//                
                ...
                nova.setId(infoselecionada.getId());
                
                nova.setNome(infoselecionada.getNome());
                
                nova.setPeriodo(infoselecionada.getPeriodo());
                
                ctrlInterface.setMedicao(nova);
                
                ctrlInterface.carregarListaClassificacao();
...
//                }
                ...
                this.ctrlInterface.habilitarAbaVisaoAuxiliar();
                this.ctrlInterface.auxiliarFromRecuperar();
                this.ctrlInterface.atualizarTabelaVisaoAuxiliar();
                this.ctrlInterface.abrirAbaVisaoAuxiliar();
                this.ctrlInterface.desabilitarAbaRecuperarprocessados();
            }
