...
public class AnaliseMensalDao {
    ...
    public void gravarAnaliseMensal (Connection conexao, AnaliseMensal analise, Sensor sensor, Info estudo){
        try {
            String estacionariedade, idAnaliseMensal,idSensor, idEstudo, tendencia, coefSperman, mes, qtdNulls, qtdSubs, qtdElem, desvAbsmed, errAbsMed, errAbsMedper, insert;
            ...
            estacionariedade =  analise.getEstacionariedade();
            ...
            insert = ""
                    ...
                    + "INSERT INTO tb_relatorio_coeficientes(id,id_estudo,id_sensor,mes,coef_sperman,tendencia,estacionariedade) VALUES('"+idAnaliseMensal+"','"+idEstudo+"','"+idSensor+"','"+mes+"',"+coefSperman+","+tendencia+",'"+estacionariedade+"');\n"
    ...
    public List<AnaliseMensal> getAnalisemensalEstudoSensor(Connection conexao, String idEstudo, String idSensor){
        try {
            ...
            System.out.println("SELECT AM.*, AE.*, AC.*\n"
                    + "FROM tb_relatorio_mensal AS AM\n"
                    + "LEFT JOIN tb_relatorio_erro AS AE\n"
                    + "ON AM.id = AE.id\n"
                    + "LEFT JOIN tb_relatorio_coeficientes as AC\n"
            ...
            while (result.next()){
                ...
                nova.setCoefSperman("coef_sperman");
                nova.setEstacionariedade(result.getString("estacionariedade"));
                nova.setMad(result.getString("desv_abs_medio"));
                nova.setMae(result.getString("erro_abs_medio"));
                nova.setMape(result.getString("erro_abs_medio_per"));
                nova.setQtdElementos(result.getInt("qtd_elem_mes"));
                nova.setQtdNull(result.getInt("qtd_nulls_mes"));
                nova.setQtdSubs(result.getInt("qtd_subs_mes"));
                nova.setTendencia(result.getString("tendencia"));
