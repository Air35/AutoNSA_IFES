...
public class ListaClassificacaoDAO {
    ...
    public List<ListaClassificacao> getListaClassificacao(Connection conexao, Info Estudo){

        ArrayList<ListaClassificacao> lista;

        ListaClassificacao nova;

        lista = new ArrayList<ListaClassificacao>();


        for (int i=0;i<Estudo.getLista().size(); i++){
            nova = new ListaClassificacao(Estudo.getColuna(i));
            nova.setAnaliseMensal(this.ctrldao.getListaAnaliseMensalEstudoSensor(Estudo.getId(), Estudo.getLista(i).getSensor().getId()));
            lista.add(nova);
        }
